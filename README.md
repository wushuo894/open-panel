...
- 大封面与列表两种页面模式，均可展示内置 WebP 壁纸，并支持背景上传、多壁纸轮换、可调遮罩和自定义主题色；主题色可从当前壁纸自动选取
- 可配置 Banner 时间、秒数、日期、周几、标题、一句话及最多四行页脚
- 卡片分组支持仅图标、图标 + 标题 + 备注两种卡片样式，并可选择纵向分组或 Tabs 切换布局；卡片与搜索框支持统一调整透明度和圆角
...
- 设置页在 Docker 可用时显示容器管理页，可查看运行时间、启停或重启容器、查看对应的 `docker-compose.yaml`，并清理未被容器引用的镜像；项目启动后及每 3 小时只查询远端镜像摘要，不会拉取镜像或重建容器；只有手动点击更新按钮才会拉取镜像并更新容器，同时显示实时进度与更新日志
...
- ZIP 配置备份与恢复，可同时迁移上传的背景图和卡片图标；支持 PWA 安装与公开页面离线缓存
...
默认监听 `0.0.0.0:57788`，配置目录为当前路径下的 `config`。可通过环境变量修改：
...
开发服务器地址为 `http://localhost:37788`，`/api` 会代理至 `http://127.0.0.1:57788`。
...
- ZIP 配置备份包含服务凭据、登录密钥及上传图片，应按敏感文件保管。
