...
- 系统卡片展示 CPU、内存、存储和网络信息
- 服务卡片支持 Emby、ani-rss、qBittorrent 的 API Key 专属状态采集，并保留通用 Web 探活
- Docker 卡片展示指定容器的运行状态、CPU、内存和运行时间；已登录管理员可启动、停止或重启容器
- 管理员可扫描指定 IP/域名的 80-65535 端口，仅保留 HTTP 200 的 HTML 服务，识别标题、图标和介绍后批量添加，并自动跳过 Open Panel 自身
...
软件服务填写访问地址后即可作为导航打开；填写 API Key 后，进入首页时会并行采集一次专属摘要：Emby 显示播放与转码，ani-rss 显示订阅和下载任务，qBittorrent 显示版本、下载数和做种数。API Key 只由后端请求服务 API，不会随公开面板配置返回。
